...
float RobotisationAudioProcessor::getParameter (int index)
{
    ...
    switch (index)
    {
		...
		case kfftFilterParam:  return (bool)fftFilter_;
        default:               return 0.0f;
    }
}
...
void RobotisationAudioProcessor::setParameter (int index, float newValue)
{
    ...
    switch (index)
    {
        ...
        case kWindowTypeParam:
            ...
            break;
		// Paulo: set Text button parameter
		case kfftFilterParam:
			fftFilter_ = (bool)newValue;
			//if (preparedToPlay_)
				//updateHopSize();
			break;
        default:
...
const String RobotisationAudioProcessor::getParameterName (int index)
{
    switch (index)
    {
		...
		case kfftFilterParam:  return "fft filter";
        default:               break;
    }
...
void RobotisationAudioProcessor::prepareToPlay (double sampleRate, int samplesPerBlock)
{
	...
	samplerate = (float)getSampleRate(); // Paulo: get sample rate
    initFFT(fftSelectedSize_);
    initWindow(fftSelectedSize_, windowType_);
    ...
    for (channel = 0; channel < numInputChannels; ++channel)
    {
        ...
        for (int i = 0; i < numSamples; ++i)
        {
            ...
            if (++sampsincefft >= hopActualSize_)
            {
				...
				// Paulo: Telephone channel FFT filter 
				if (fftFilter_) {
					int lowFreqBin = 1200 * fftActualTransformSize_ / samplerate;
					int hiFreqBin = 3200 * fftActualTransformSize_ / samplerate;

					for (int bin = 0; bin <= fftActualTransformSize_ / 2; bin++)
					{
						if (bin < lowFreqBin || bin > hiFreqBin) {
							fftFrequencyDomain_[bin][0] = 0;
							// We need to maintain that symmetry to produce a real output
							if (bin > 0 && bin < fftActualTransformSize_ / 2) {
								fftFrequencyDomain_[fftActualTransformSize_ - bin][0] = 0;
							}
						}
					}
				}
...
void RobotisationAudioProcessor::getStateInformation (MemoryBlock& destData)
{
	...
	xml.setAttribute("fftFilter", fftFilter_);
...
void RobotisationAudioProcessor::setStateInformation (const void* data, int sizeInBytes)
{
    ...
    if(xmlState != 0)
    {
        ...
        if(xmlState->hasTagName("C4DMPLUGINSETTINGS"))
        {
			...
			fftFilter_ = (bool)xmlState->getDoubleAttribute("fftFilter", fftFilter_);
