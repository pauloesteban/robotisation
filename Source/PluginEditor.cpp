      ...
      resizer_ (std::make_unique <ResizableCornerComponent> (this, &resizeLimits_))
{
	...
	// Paulo: set up the on/off button
	addAndMakeVisible(&fftFilterTextButton_);
	fftFilterTextButton_.setButtonText(TRANS("PHONE"));
	fftFilterTextButton_.addListener (this);
	fftFilterTextButton_.setColour(TextButton::buttonColourId, Colours::white);
	fftFilterTextButton_.setColour(TextButton::buttonOnColourId, Colours::chartreuse);
...
void RobotisationAudioProcessorEditor::resized()
{
	...
	fftFilterTextButton_.setBounds(200, 70, 150, 30); // Paulo: TextButton position
...
void RobotisationAudioProcessorEditor::timerCallback()
{
	...
	// Paulo: Display the TextButton in its correct state
	if (fftFilterTextButton_.getToggleState()) fftFilterTextButton_.setToggleState(true, false);
	else fftFilterTextButton_.setToggleState(false, false);
}
...
//Paulo: callback for button updates
void RobotisationAudioProcessorEditor::buttonClicked(Button* buttonThatWasClicked)
{
	//[UserbuttonClicked_Pre]
	//[/UserbuttonClicked_Pre]

	if (buttonThatWasClicked == &fftFilterTextButton_)
	{
		//[UserButtonCode_onOff] -- add your button handler code here..
		//[/UserButtonCode_onOff]
		getProcessor()->setParameterNotifyingHost(RobotisationAudioProcessor::kfftFilterParam,
			                                      (bool)fftFilterTextButton_.getToggleState());
	}
